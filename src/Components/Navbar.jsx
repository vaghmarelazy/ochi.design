...
function Navbar() {
  ...
  return (
    <div style={{
      ...
      zIndex: 999, // Ensure navbar is above the blurred content
      // backgroundColor:'red'
    }} className="fixed z-[999] w-full px-10 py-8 flex justify-between">
      ...
      <div className="links flex gap-10 shead font-light overflow-hidden">
        {["Services", "Our Work", "About Us", "Insights", "Contact Us"].map(
          (item, index) => (
            <a
              key={index}
              className={`navbar shead text-lg font-light cursor-pointer relative ${index===4 && "ml-32"}`}
            ...
            >
              ...
              {hoveredIndex === index && (
                <div
                  className={`underline absolute left-0 bottom-0 bg-white h-[1px] `}
                  style={{
                    width: "100%",
                    transition: "0.3s ease-in-out",
                    transformOrigin: "left",
                    transform: "scaleX(1)",
                    filter: isVisible ? 'blur(10px)' :'none',
                    zIndex:2
                  }}
                ></div>
              )}
